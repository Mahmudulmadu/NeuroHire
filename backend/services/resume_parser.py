...
def _detect_file_type(file_data: bytes, filename: str) -> Optional[str]:
    ...
    if magic is not None:
        try:
            ...
            log_warning(f'libmagic unavailable, falling back to signature detection: {exc}', context='resume_parser')
    ...
    if suffix == '.docx' and file_header.startswith(b'PK'):
        return 'docx'
...
def validate_file(file_data:bytes, filename:str)->Tuple[bool, str, Optional[str]]:
    ...
    if file_type is None:
        supported = ', '.join(sorted(set(SUPPORTED_MIME_TYPES.values())))
        return False, (
