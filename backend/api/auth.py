...
# When True, if no Bearer token is provided the backend accepts the x-user-id header
# as the authenticated identity. NEVER enable in production.
_LOCAL_DEV_MODE = os.getenv('NEUROHIRE_LOCAL_DEV', '').lower() in ('1', 'true', 'yes')
...
def get_current_user(
    ...
    # Local dev fallback: accept x-user-id header when no token is present
    if (creds is None or not creds.credentials) and _LOCAL_DEV_MODE:
        user_id = request.headers.get('x-user-id', '00000000-0000-0000-0000-000000000000')
        logger.debug(f'Local dev auth: using x-user-id={user_id}')
        return user_id
